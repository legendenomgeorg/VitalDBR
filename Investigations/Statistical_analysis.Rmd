...
* preop_dm
  + Whether the patient has too high bloodpressure
...
We will refrain from interpret coefficients, before we have decided on the best model. So to determine whether our model violates its assumptions. For the linear regression we do a classic plot of the residuals against the dependent variable and a qqplot
...
This in itself is not that useful, but now the distribution is defined from it's mean, and because the GLM models the mean of a regression this allows us to use the negative binomial distribution as our family.
...
In our case, this formula can be interpreted with an example. Let's say we are looking at the influence of our independent variables on patients spending 29 days in the ICU ($Y=29$). Then this is comprised of two different stochastic variables. T which determines the number of cases where the patients spend 29 days in the ICU. In our case, the number of patients is given by the poisson distribution. 
...
To visualize this, here is the barplot from the beginning, but now imagine that the vertical value is decided by a poisson distribution and the horizontal value is given by the gamma distribution. Then the joint distribution of those variables is the compound poisson distribution.


```{r}
count <- table(factor(icu, levels = 0:81))
barplot(count, xlab = "X ~ Gamma(alpha, beta)", ylab = "T ~ poisson(lambda)")
```
