...
This calls for some non-standard analysis. But in order to asses the performance of our model, we need to first of all make a baseline. We do that with a simple poisson regression
...
If we would like a graphical view of how the distributions overlay the histogram, then the regular poisson and the compound poisson is trivial. But for the zero inflated model, there exists no parametric way to define the density function on a given dataset. 
...
```{r}
# It is a bit cumbersome to install but here is what you need:
#-----------------------
