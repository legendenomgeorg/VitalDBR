...
To visualize this, lets look at the barplot from the beginning, but now imagine that the vertical value is decided by a poisson distribution and the horizontal value is given by the gamma distribution. Then the compound poisson process is obtained by marginalizing over T.
```{r}
...
If we had access to the specific type of surgery, and a doctor that could determine whether they were dangerous or not, we could more accurately account for the confounder bias in our regression. Perhaps even divide into more levels of how likely that operation is to send you to the icu. This would in turn require more data, since we are balancing just around the point where we can't add any more variables to our zero inflated regressions, due to the size of the model. 
