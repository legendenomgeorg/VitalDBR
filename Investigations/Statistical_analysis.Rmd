...
## Skal lige skrives naturligt ind
...
GLM's don't assume homoskedacity, as the variance 
...
The negative binomial distribution is similar to the poisson, but it has a dispersement parameter, that influences the variance. 
If Y us distributed as 
$$Y \sim NB(r,p) ={k+r-1 \choose k} \cdot (1-p)^r p^k $$
where r is the number of failures until the experiment is stopped and p is the probability of success, the expected value and variance can be defined as follows:

$$ E[Y] = \frac{pr}{1-p}$$
$$ Var[Y] = \frac{pr}{(1-p)^2}$$
But the negative binomial distribution can be reformulated as: 
$$\operatorname{Pr}(X=k)=\frac{\Gamma(r+k)}{k ! \Gamma(r)}\left(\frac{r}{r+m}\right)^{r}\left(\frac{m}{r+m}\right)^{k} \quad \ \ k=0,1,2, \ldots$$
(der er en ok forklaring på det her på wikipedia, skal jeg gå mere i dybden?)
https://en.wikipedia.org/wiki/Negative_binomial_distribution
And the expected value and variance can then be derived as:
$$E[Y] = m $$
$$Var[Y] = m+\frac{m^2}{r} $$
Where $\alpha = \frac{1}{r}$ is the dispersion parameter in the GLM output below, which controls the variance. 
Now, when fitting the negative binomial we can express it in terms of it's mean through a log link and $r$ , which means that the formula then becomes:
$$Y \sim NB(e^{\beta X}, r)  $$
...
There are various formulations we can use, depending on how we believe the variables influence each part of the model:

```{r}
pscl::zeroinfl(icu_days ~ . , data=data, dist="poisson")
```

If we believe that the variables both influence the zero inflation AND the poisson distribution
...
We also add all variables, that are not a result of the operation. This is because they do not indicate anything about the operation, but instead about the general condition of the patient. These are every variable but: ppv_under_5, ppv_under_8 and ppv_avg_last_30 .
(her burde vi adde knifte_time, men så er der for mange variable i modellen ift data :i ) 
...
## Calculating metrics
Caculating AICc and BIC for the Compound poisson regression has to be done manually for some reason
```{r}
...
logLik_tweedie <-
  function(cpglm_obj) {
    ...
    dens <- tweedie::dtweedie(y = cpglm_obj$y, 
                              ...
                              xi = cpglm_obj$p, 
                              phi = cpglm_obj$phi)
    # we add 2 to the number of parameters, since the dispersion was estimated for computing
...
(Skal skrive mere grundigt)
Operation type is definitely a confounder in our analysis. Some operations will almost certainly make you stay in the icu for over 0 days, and some operations are just simple operations, that will almost never result in an ICU stay. 
For all of our "preop" variables, it would be of great benefit to jhave a corresponding "postop" variable. 
