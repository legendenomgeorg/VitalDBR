...
As stated in the introduction, we want to look into the importance of the Pulse Pressure Variation (PPV) on whether patient's time spent in the ICU (intensive care unit)
...
A standard linear regression is actually just a GLM with the "gaussian family" ie. errors normally distributed and the identity function as the link function!
...
Another reason we use GLM's is that they don't assume homoskedacity, as 
...
But the negative binomial distribution can also be formulated as a poisson distribution with an added Gamma noise variable $r$ which has mean 1 and a scale parameter $v$.
...
This in itself is not that useful, but it means that now we can define the mean and variance of the negative binomial distribution as:
$$E[Y] = m $$
$$Var[Y] = m+\frac{m^2}{r} $$
Where $\alpha = \frac{1}{r}$ is the dispersion parameter in the GLM output below, which controls the variance. Note that as this dispersion paramter gets larger and larger, the variance converges to the same value as the mean, and then the negative binomial distribution magically turns into the poisson distribution. This means that we now have a distribution that is similar to the poisson, but we can vary the variance, this is perfect since we can account for overdispersion by changing $r$
...
We also add all variables, that are not a result of the operation. This is because they do not indicate anything about the operation, but instead about the general condition of the patient before the operation. These are every variable but: ppv_under_5, ppv_under_8 and ppv_avg_last_30 .
