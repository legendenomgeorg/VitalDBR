...
ppv_prepare <- function(case, start, end, data_art, data_awp){
  ...
  for (i in 0:iterations){
    ...
    if (nrow(sub_awp)<=2000){ 
      data[i+1] <- NA
      next
    }
    insp_start <- VitalDBR::get_inspiration_start(sub_awp)
    sub_art <- VitalDBR::subset_data(data = data_art, seconds = interval, start_sec = start+(interval*i))
    
    if (sum(is.na(sub_art))>=nrow(sub_art)/2){ 
      # if the dataframe consists of over half na values we skip the iteration
      data[i+1] <- NA
      next
    }
    
    # had to not filter with the subset function, due to too many NA values...
    hz <- 1/(sub_art[2,1]-sub_art[1,1])
    sub_art <- na.omit(sub_art)
    sub_art <- waveformtools::filter_signal(sub_art, 25, sample_rate = hz, signal_col = 2)
    #cat("sub_awp",nrow(sub_awp))
    #cat("sub_art",nrow(sub_art))
    beats <- waveformtools::find_abp_beats(sub_art,abp_col=3, time_col=1)
    if (nrow(beats)<=40){  #the constant is based on amount of parameters in the gam
      data[i+1] <- NA
      next
    }
...
process_cases <- function(data){
  ...
  for (caseid in data$caseid){
    ...
    ppv_under6 <- rbind(ppv_under6, sum(ppv_results<=5)/len_ppv)
    ppv_over12 <- rbind(ppv_over12, sum(ppv_results>=8)/len_ppv)
  }
  colnames(ppv_under6) <- c('ppv_under5')
  colnames(ppv_over12) <- c('ppv_over8')
...
path <- getwd()
#filename = paste(path, 'df_201_250.csv',sep = '')
